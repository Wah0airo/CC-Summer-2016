...
=======
    ...
    *(SYMBOLS + SYM_IDENTIFIER)   = (int) "identifier";
>>>>>>> 3c7d131c8612fde8a1b86a81f4d12f868b9db305
...
=======
void declare_array(int* id, int typ, int off);
int  write_array(int* name);
void global_array(int* id, int type);
int  gr_call(int* procedure);
>>>>>>> 3c7d131c8612fde8a1b86a81f4d12f868b9db305
...
<<<<<<< HEAD
    ...
    a = i / SIZEOFINT;
    //temp = rightShift(leftShift(*(s + a), ((SIZEOFINT - 1) - (i % SIZEOFINT)) * 8), (SIZEOFINT - 1) * 8);
...
int currentTemporary() {
    ...
    if (allocatedTemporaries > 0)
        ...
        syntaxErrorMessage((int*) "current illegal register access");
...
int previousTemporary() {
    ...
    if (allocatedTemporaries > 1)
        ...
        syntaxErrorMessage((int*) "previous illegal register access");
...
void tfree(int numberOfTemporaries) {
    ...
    //print((int*)"tfree: allocatedTemporaries: ");
    //printString(itoa(allocatedTemporaries, string_buffer, 10, 0, 0));
    //print((int*)"numberOfTemporaries: ");
    //printString(itoa(allocatedTemporaries, string_buffer, 10, 0, 0));
    //println();
...
void declare_array(int *id, int typ, int off) {
    ...
    i = 0;
    //int* id;
    //print((int*) "array symbol ");
    //print(literal);
    //println();
    print((int*)"declare_array ");
    printSymbol(symbol);
    ...
    if (symbol == SYM_INTEGER) {
        //print((int*) "array index: ");
        //print(literal);
        //println();
        if(literal > 0) {

          if(off >= 0){
            tempsize = off + ( literal * SIZEOFINT);
            ...
            //list = malloc(literal*SIZEOFINT);
            print((int*) "literal: ");
            printString(itoa(literal, string_buffer, 10, 0, 0));
            println();

            createSymbolTableEntry(LOCAL_TABLE, id, lineNumber, VARIABLE, typ, 0, tempsize);
            //while(i < literal) {
            //  *(list + i) = touch(*(list + i), SIZEOFINT);
            // i = i + 1;
            //}
            getSymbol();
            print((int*)"literal > 0 ");
...
void read_write_array(int* name) {
    ...
    if(symbol == SYM_IDENTIFIER) {
        name_of_identifier = identifier;
        is_identifier = 1;
    }
    
    sh_expr = gr_shiftExpression();
    
    if(sh_expr == SYM_INTEGER) {
        if(is_identifier==1) {
            ident_value = getSymbolTableEntry(name_of_identifier, VARIABLE);//
            value_of_ident = getValue(ident_value);
            index = value_of_ident;
            is_identifier = 0;
        }
        else {
            index = literal;
        ...
        if(symbol == SYM_RBRACKET) {
            ...
            if(symbol == SYM_ASSIGN) {
                ...
                if(symbol == SYM_INTEGER) {
                    load_integer(literal);
                    index = index * 4;
                    address_array = address_array + index;
                    
                    emitIFormat(OP_SW, getScope(array_name), currentTemporary(), address_array);
                    tfree(1);
                    ...
                    syntaxErrorMessage((int*) "array integer expected");
                }
            }
...
int write_array(int* name) {
    ...
    list = searchSymbolTable(local_symbol_table, name, VARIABLE);
    if(list == (int*) 0)
        list = getSymbolTableEntry(name, VARIABLE);
    else {
        list = searchSymbolTable(global_symbol_table,name,VARIABLE);
        if(list == (int*) 0)
          list = getSymbolTableEntry(name, VARIABLE);
        else{
          syntaxErrorMessage((int*) "array name not declared");
...
<<<<<<< HEAD
        syntaxErrorMessage((int*) "array SYM_INTEGER integer expected");
    }
...
void global_array(int* id, int type){
    ...
    if(symbol == SYM_INTEGER){
        if(literal > 0){
          ...
          createSymbolTableEntry(GLOBAL_TABLE, id, lineNumber, VARIABLE, type, 0, -allocatedMemory);
          getSymbol();
          if(symbol == SYM_RBRACKET){
            getSymbol();
            if(symbol == SYM_SEMICOLON)
              ...
              syntaxErrorMessage((int *) "Semicolon expected");

          }
          else
              syntaxErrorMessage((int *) "Right bracket expected");


        }
        else
          syntaxErrorMessage((int *) "integer number must be a > 0 ");


    }
    else{
      syntaxErrorMessage((int *) "must be a positive integer number ");
    }
...
=======
    ...
    ltype = gr_factor(attribute_list);
...
=======
        ...
        rtype = gr_factor(attribute_list);
...
=======
            ...
            if(symbol == SYM_LBRACKET) {
                temp = write_array(variableOrProcedureName);
                load_integer(temp);
...
<<<<<<< HEAD
            //getSymbol();
            //declare_array(variableOrProcedureName);
            
            //print((int*)"statement ident[ ");
            //printString(variableOrProcedureName);
            //println();
            read_write_array(variableOrProcedureName);
			tfree(1);
...
=======
            getSymbol();
            //declare_array(variableOrProcedureName);
            write_array(variableOrProcedureName);
            //if(symbol==SYM_SEMICOLON) {
            //    getSymbol();
            //}
            //else
...
=======
            ...
            println();
            getSymbol();
            declare_array(ident, type, offset);
        }
        else
            createSymbolTableEntry(LOCAL_TABLE, ident, lineNumber, VARIABLE, type, 0, offset);
...
=======
    ...
    printSymbol(SYM_LBRACKET);
    printSymbol(SYM_RBRACKET);
    //Test Cases

    list = malloc(10*4);
    //print((int*)"Testing arrays");
    //println();
    //while(i<10) {
      //  printString(itoa(i, string_buffer, 10, 0, 0));
      //  print((int*)" ");
        //temp =touch(malloc(SIZEOFINT), SIZEOFINT);
        //temp = *(list + i);
       // mapPage(list, i, SIZEOFINT);
        //storePhysicalMemory
        //i = i + 1;
    //}
    i = 0;
