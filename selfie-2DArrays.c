...
void initScanner () {
    ...
    SYMBOLS[0][0]      = (int) "identifier";
    SYMBOLS[1][0]     = (int) "integer";
    SYMBOLS[2][0]     = (int) "void";
    SYMBOLS[3][0]      = (int) "int";
    SYMBOLS[4][0]      = (int) ";";
    SYMBOLS[5][0]      = (int) "if";
    SYMBOLS[6][0]      = (int) "else";
    SYMBOLS[7][0]      = (int) "+";
    SYMBOLS[8][0]      = (int) "-";
    SYMBOLS[9][0]      = (int) "*";
    SYMBOLS[10][0]     = (int) "/";
    ...
    SYMBOLS[15][0]     = (int) "{";
    SYMBOLS[16][0]     = (int) "}";
    SYMBOLS[17][0]     = (int) "while";
    SYMBOLS[18][0]     = (int) "return";
    SYMBOLS[19][0]     = (int) ",";
    SYMBOLS[20][0]     = (int) "<";
    SYMBOLS[21][0]     = (int) "<=";
    SYMBOLS[22][0]     = (int) ">";
    ...
    SYMBOLS[24][0]     = (int) "!=";
    SYMBOLS[25][0]     = (int) "%";
    SYMBOLS[26][0]     = (int) "character";
    SYMBOLS[27][0]     = (int) "string";
    SYMBOLS[28][0]     = (int) "<<";
    SYMBOLS[29][0]     = (int) ">>";
    SYMBOLS[30][0]     = (int) "[";
    SYMBOLS[31][0]     = (int) "]";
    SYMBOLS[32][0]     = (int) "struct";
    SYMBOLS[33][0]     = (int) "->";
    SYMBOLS[34][0]     = (int) "!";
    SYMBOLS[35][0]     = (int) "&&";
    SYMBOLS[36][0]     = (int) "||";
...
int  getCols(int* entry)       {
    return *(entry + 9);
}
int* getStruct(int* entry)     {
    return (int*) *(entry + 10);
}
...
void setCols(int* entry, int cols)          {
    *(entry + 9) = cols;
}
void setStruct(int* entry, int* this_struct) {
    *(entry + 10) = (int) this_struct;
}
...
void gr_statement();
int  gr_type();
void gr_variable(int offset, int* new_struct);
        ...
        if(literal > 0) {
            ...
            if(symbol == SYM_RBRACKET) {
                ...
                if(symbol == SYM_SEMICOLON) {
                    ...
                    if(whichTable == GLOBAL_TABLE) {
                        //size_array = index_1D * WORDSIZE;
                        offset_array = offset_loc + size_array; print((int* )"####################");println();
                        ...
                        offset_array = offset_loc + WORDSIZE  - size_array;//print((int* )">><<>><<>><<>><<>><<>><<");println();
                        createSymbolTableEntry(whichTable, id, lineNumber, ARRAY, typ, 0, offset_array, size_array, 0, (int*) 0);
                    }
...
int gr_factor(int* constFold) {
    ...
    if (symbol == SYM_ASTERISK) {
        ...
        //Array zugriff variable = array[];
            ...
            if (symbol == SYM_RBRACKET) {
                ...
                if(symbol==SYM_LBRACKET) {
                    ...
                    // print((int*)itoa(getCols(entry), string_buffer, 10, 0, 0));
                    // print((int*)" || ");printString((getString(entry)));
                    // println();
                    // printLineNumber((int*) " ln ", lineNumber);
                    // println();

                    load_integer(getCols(entry));
                  //  load_integer(getSize(entry)); //
        ...
        //println();
        //  if(symbol==SYM_LBRACKET) {
        //      type = access_array(variableOrProcedureName);
        //  }
        //  else {
        //      type = load_variable(variableOrProcedureName);
        //  }
...
void gr_statement() {
    ...
    //TODO array
        ...
        //ZUweisung 2D array
            ...
            emitLeftShiftBy(2);
            emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(), getAddress(entry));
            // print((int*)"Address: "); print((int*)itoa(getAddress(entry),string_buffer,16,0,0)); println();
            emitRFormat(OP_SPECIAL, currentTemporary(), getScope(entry), currentTemporary(),  FCT_ADDU);
            //Array Assignment (write slot): identifier "[" expression "]" = expression
            if (symbol == SYM_ASSIGN) {
                getSymbol();
                //print((int*)"statement, id[]= ");
                //printString(getString(entry));
                //print((int*) " Address= ");
                //print(itoa(getAddress(entry), string_buffer, 10, 0, 0));
                //printLineNumber((int*) " ln: ", lineNumber);
                ...
                if (symbol == SYM_LPARENTHESIS)
                    ...
                    if(symbol == SYM_INTEGER) {
                        ...
                        if(symbol == SYM_RBRACKET) {
                            ...
                            else if(symbol == SYM_LBRACKET) { ///2D
                                getSymbol();
                                if(symbol == SYM_INTEGER) {
                                    if(literal > 0) {
                                        index_2D = literal;
...
void global2DArrayTest(){
    ...
    print((int*)"Zuweisung local an den Globalen 2D array");
    println();
    gza[0][0] = 2;
    gza[0][1] = 4;
    gza[0][2] = 8;
...
void testArrays(){
    ...
    int h;    int i;    int arr[8];    int j;    int k;   int l;
    ...
    while(idx < 10) {
        print((int*)"garr[");
        print(itoa((int*)idx,string_buffer,10,0,0));
        print((int*) "]: ");
        print(itoa((int*)garr[idx],string_buffer,10,0,0));
        print((int*) " ");
        idx = idx + 1;
        println();
    }
    ...
    while(idx < 8) {
        ...
        print(itoa((int*)idx,string_buffer,10,0,0));
        print((int*) "]: ");
        print(itoa((int*)arr[idx],string_buffer,10,0,0));
        print((int*) " ");
        idx = idx + 1;
        println();
    }
